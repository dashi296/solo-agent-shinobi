...
- lock 取得後に branch 作成までは成功し、その後の state 更新が失敗した場合に限って、その mission を `start` 未完了の local-only mission として retryable に残し、state に `retryable_local_only: true` と失敗理由を記録する
