...
- `.shinobi/review-notes.md` を review 知見の蓄積先とする
- 記録するのは指摘本文そのものではなく、再発防止ルールと確認観点とする
- 新しい Issue の着手前に、関連カテゴリの notes を確認する
- PR 前にセルフレビューを行い、少なくとも次を確認する
  - 目的を満たしているか
  - 完了条件を yes/no で満たしているか
  - スコープ外を混ぜていないか
  - 失敗時 cleanup があるか
  - 正常系と失敗系の両方を検証したか
- 同じ指摘が 2 回以上出たら、notes だけで済ませず test / lint / helper / template のいずれかへ昇格する

推奨する `review-notes.md` の項目例:
