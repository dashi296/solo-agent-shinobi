...
context / execute / review loop が実装された後は、実装前に `.shinobi/review-notes.md` 全体を読むのではなく、今回の task に関連するカテゴリだけを確認します。関連カテゴリが不明確な場合でも、確認対象は最大 2 カテゴリまでに抑えます。

同じく将来の実装では、PR 前セルフレビューに `.shinobi/templates/self-review.md` を使います。review で新しい指摘を受けた場合は、必要に応じて `.shinobi/templates/review-note-rule.md` に沿って `.shinobi/review-notes.md` へ再発防止ルールを追記します。
