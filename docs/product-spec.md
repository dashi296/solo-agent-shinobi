...
- local-only mission を resume してよいのは、state に保存した `agent_identity`, `run_id`, `issue_number`, `branch`, `phase` が branch 実体と整合し、かつその `agent_identity` が現在設定の一意な `agent_identity` と一致し、`retryable_local_only: true` が残り、branch 作成後の retryable な `start` 失敗記録で裏付けられる場合に限る
